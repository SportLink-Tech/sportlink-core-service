...
func TestCreateMatchOffer(t *testing.T) {
	validator := validator.New()

	testCases := []struct {
		name    string
		payload request.NewMatchOfferRequest
		on      func(t *testing.T, useCaseMock *UseCaseMock)
		then    func(t *testing.T, responseCode int, response map[string]interface{})
	}{
		{
			name: "given_valid_match_offer_when_creating_then_returns_created_offer",
			payload: request.NewMatchOfferRequest{
				TeamName: "Boca",
				Sport:    "Paddle",
				Day:      "2025-12-10",
			...
			on: func(t *testing.T, useCaseMock *UseCaseMock) {
				expectedEntity := domain.NewMatchOffer(
					"Boca",
					common.Paddle,
		...
		{
			name: "given_specific_categories_when_creating_then_returns_created_offer",
			payload: request.NewMatchOfferRequest{
				TeamName: "Boca",
				Sport:    "Paddle",
				Day:      "2025-12-10",
		...
		{
			name: "given_between_categories_when_creating_then_returns_created_offer",
			payload: request.NewMatchOfferRequest{
				TeamName: "Boca",
				Sport:    "Paddle",
				Day:      "2025-12-10",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			controller := matchoffer.NewController(useCaseMock, nil, nil, nil, nil, nil, validator)
			...
			req, _ := http.NewRequest("POST", "/account/test-account-id/match-offer", bytes.NewBuffer(jsonData))
