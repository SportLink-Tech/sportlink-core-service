...
func TestDefaultQueryParser_Statuses(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name:      "single status",
			input:     "PENDING",
			want:      []matchoffer.Status{matchoffer.StatusPending},
			wantError: false,
		},
		{
			name:      "multiple statuses",
			input:     "PENDING,CONFIRMED",
			want:      []matchoffer.Status{matchoffer.StatusPending, matchoffer.StatusConfirmed},
			wantError: false,
		},
		{
			name:      "statuses with spaces",
			input:     "PENDING, CONFIRMED , CANCELLED",
			want:      []matchoffer.Status{matchoffer.StatusPending, matchoffer.StatusConfirmed, matchoffer.StatusCancelled},
			wantError: false,
		},
		{
			name:      "invalid status",
...
func equalStatuses(a, b []matchoffer.Status) bool {
	if len(a) != len(b) {
		return false
	}
	for i := range a {
