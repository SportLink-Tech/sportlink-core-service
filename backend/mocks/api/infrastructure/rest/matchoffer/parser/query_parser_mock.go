// Code generated by mockery v2.46.1. DO NOT EDIT.

package mocks

import (
	common "sportlink/api/domain/common"
	matchoffer "sportlink/api/domain/matchoffer"
	"time"

	mock "github.com/stretchr/testify/mock"
)

// QueryParser is an autogenerated mock type for the QueryParser type
type QueryParser struct {
	mock.Mock
}

// Categories provides a mock function with given fields: categoriesQuery
func (_m *QueryParser) Categories(categoriesQuery string) ([]common.Category, error) {
	ret := _m.Called(categoriesQuery)

	if len(ret) == 0 {
		panic("no return value specified for Categories")
	}

	var r0 []common.Category
	var r1 error
	if rf, ok := ret.Get(0).(func(string) ([]common.Category, error)); ok {
		return rf(categoriesQuery)
	}
	if rf, ok := ret.Get(0).(func(string) []common.Category); ok {
		r0 = rf(categoriesQuery)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]common.Category)
		}
	}

	if rf, ok := ret.Get(1).(func(string) error); ok {
		r1 = rf(categoriesQuery)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// Date provides a mock function with given fields: dateQuery
func (_m *QueryParser) Date(dateQuery string) (time.Time, error) {
	ret := _m.Called(dateQuery)

	if len(ret) == 0 {
		panic("no return value specified for Date")
	}

	var r0 time.Time
	var r1 error
	if rf, ok := ret.Get(0).(func(string) (time.Time, error)); ok {
		return rf(dateQuery)
	}
	if rf, ok := ret.Get(0).(func(string) time.Time); ok {
		r0 = rf(dateQuery)
	} else {
		r0 = ret.Get(0).(time.Time)
	}

	if rf, ok := ret.Get(1).(func(string) error); ok {
		r1 = rf(dateQuery)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// Location provides a mock function with given fields: country, province, locality
func (_m *QueryParser) Location(country, province, locality string) *matchoffer.Location {
	ret := _m.Called(country, province, locality)

	if len(ret) == 0 {
		panic("no return value specified for Location")
	}

	var r0 *matchoffer.Location
	if rf, ok := ret.Get(0).(func(string, string, string) *matchoffer.Location); ok {
		r0 = rf(country, province, locality)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*matchoffer.Location)
		}
	}

	return r0
}

// Sports provides a mock function with given fields: sportsQuery
func (_m *QueryParser) Sports(sportsQuery string) ([]common.Sport, error) {
	ret := _m.Called(sportsQuery)

	if len(ret) == 0 {
		panic("no return value specified for Sports")
	}

	var r0 []common.Sport
	var r1 error
	if rf, ok := ret.Get(0).(func(string) ([]common.Sport, error)); ok {
		return rf(sportsQuery)
	}
	if rf, ok := ret.Get(0).(func(string) []common.Sport); ok {
		r0 = rf(sportsQuery)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]common.Sport)
		}
	}

	if rf, ok := ret.Get(1).(func(string) error); ok {
		r1 = rf(sportsQuery)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// Statuses provides a mock function with given fields: statusesQuery
func (_m *QueryParser) Statuses(statusesQuery string) ([]matchoffer.Status, error) {
	ret := _m.Called(statusesQuery)

	if len(ret) == 0 {
		panic("no return value specified for Statuses")
	}

	var r0 []matchoffer.Status
	var r1 error
	if rf, ok := ret.Get(0).(func(string) ([]matchoffer.Status, error)); ok {
		return rf(statusesQuery)
	}
	if rf, ok := ret.Get(0).(func(string) []matchoffer.Status); ok {
		r0 = rf(statusesQuery)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]matchoffer.Status)
		}
	}

	if rf, ok := ret.Get(1).(func(string) error); ok {
		r1 = rf(statusesQuery)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// Limit provides a mock function with given fields: limitQuery
func (_m *QueryParser) Limit(limitQuery string) (int, error) {
	ret := _m.Called(limitQuery)

	if len(ret) == 0 {
		panic("no return value specified for Limit")
	}

	var r0 int
	var r1 error
	if rf, ok := ret.Get(0).(func(string) (int, error)); ok {
		return rf(limitQuery)
	}
	if rf, ok := ret.Get(0).(func(string) int); ok {
		r0 = rf(limitQuery)
	} else {
		r0 = ret.Get(0).(int)
	}

	if rf, ok := ret.Get(1).(func(string) error); ok {
		r1 = rf(limitQuery)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// Offset provides a mock function with given fields: offsetQuery
func (_m *QueryParser) Offset(offsetQuery string) (int, error) {
	ret := _m.Called(offsetQuery)

	if len(ret) == 0 {
		panic("no return value specified for Offset")
	}

	var r0 int
	var r1 error
	if rf, ok := ret.Get(0).(func(string) (int, error)); ok {
		return rf(offsetQuery)
	}
	if rf, ok := ret.Get(0).(func(string) int); ok {
		r0 = rf(offsetQuery)
	} else {
		r0 = ret.Get(0).(int)
	}

	if rf, ok := ret.Get(1).(func(string) error); ok {
		r1 = rf(offsetQuery)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// GeoFilter provides a mock function with given fields: lat, lng, radiusKm
func (_m *QueryParser) GeoFilter(lat string, lng string, radiusKm string) (*matchoffer.GeoFilter, error) {
	ret := _m.Called(lat, lng, radiusKm)

	if len(ret) == 0 {
		panic("no return value specified for GeoFilter")
	}

	var r0 *matchoffer.GeoFilter
	var r1 error
	if rf, ok := ret.Get(0).(func(string, string, string) (*matchoffer.GeoFilter, error)); ok {
		return rf(lat, lng, radiusKm)
	}
	if rf, ok := ret.Get(0).(func(string, string, string) *matchoffer.GeoFilter); ok {
		r0 = rf(lat, lng, radiusKm)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*matchoffer.GeoFilter)
		}
	}

	if rf, ok := ret.Get(1).(func(string, string, string) error); ok {
		r1 = rf(lat, lng, radiusKm)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// NewQueryParser creates a new instance of QueryParser. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewQueryParser(t interface {
	mock.TestingT
	Cleanup(func())
}) *QueryParser {
	mock := &QueryParser{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
