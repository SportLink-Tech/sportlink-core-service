...
type QueryParser interface {
	...
	Statuses(statusesQuery string) ([]matchoffer.Status, error)
	Date(dateQuery string) (time.Time, error)
	Location(country, province, locality string) *matchoffer.Location
	GeoFilter(lat, lng, radiusKm string) (*matchoffer.GeoFilter, error)
...
func (p *DefaultQueryParser) Statuses(statusesQuery string) ([]matchoffer.Status, error) {
	if statusesQuery == "" {
		return nil, nil
	}

	statusStrings := strings.Split(statusesQuery, ",")
	statuses := make([]matchoffer.Status, 0, len(statusStrings))
	...
	for _, statusStr := range statusStrings {
		...
		status, err := matchoffer.ParseStatus(trimmed)
		if err != nil {
...
func (p *DefaultQueryParser) Location(country, province, locality string) *matchoffer.Location {
	if country == "" && province == "" && locality == "" {
		return nil
	}

	return &matchoffer.Location{
		Country:  country,
		Province: province,
		Locality: locality,
	}
